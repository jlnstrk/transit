...
public class HapiEndpoint private constructor(
    ...
    public suspend inline fun <reified Res : HapiResponse> hapiRequest(request: HapiRequest<Res>): Res {
        return httpClient.get(baseUrl + request.service.service) {
    ...
    public companion object {
        public operator fun invoke(config: HapiConfig): HapiEndpoint {
            ...
            return HapiEndpoint(httpClient, config.baseUrl)
        }

        public operator fun invoke(init: HapiConfig.() -> Unit): HapiEndpoint = HapiConfig(init).let(::invoke)
    }
}